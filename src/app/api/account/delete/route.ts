...
export async function POST() {
    ...
    try {
        const { data } = await getAdmin().from('resumes').select('id').eq('user_id', uid)
    ...
    try {
        const { data: files } = await getAdmin().storage.from('resumes').list(uid)
        if (files?.length) {
            await getAdmin().storage.from('resumes').remove(files.map((f) => `${uid}/${f.name}`))
        }
    } catch (e) {
    ...
    if (resumeIds.length) {
        try {
            await getAdmin().from('resume_skills' as never).delete().in('resume_id', resumeIds)
        } catch (e) {
    ...
    for (const table of USER_ID_TABLES) {
        try {
            await getAdmin().from(table as never).delete().eq('user_id', uid)
        } catch (e) {
    ...
    try {
        await getAdmin().from('profiles' as never).delete().eq('id', uid)
    } catch (e) {
        console.warn('[account/delete] profiles:', e)
    }

    // 6. Delete the auth user itself — the part the old flow punted to "support".
    const { error: delErr } = await getAdmin().auth.admin.deleteUser(uid)
    if (delErr) {
