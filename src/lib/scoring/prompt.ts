// resuscore/src/lib/scoring/prompt.ts
//
// Shared scoring prompt + normalization — extracted verbatim from
// src/trigger/scoreJobs.ts (Plan 21 Phase 3, architecture doc §5 Layer 2)
// so the Resume Editor Agent's rescore route can reuse the exact same
// scoring logic on an edited artifact's ATS text, instead of duplicating
// (and inevitably drifting from) the SYSTEM_PROMPT. scoreJobs.ts now imports
// from here — this file has no Trigger.dev or Next.js dependency, just plain
// TS, so it's safe in both runtimes.
...
export const SYSTEM_PROMPT = `You are an expert resume-job matching AI performing precise candidate-job fit analysis.
...
export function buildPrompt(resumeText: string, job: Record<string, unknown>): string {
...
export function normalizeFastestPath(raw: Record<string, unknown>): FastestPath {
...
export function normalizeScore(raw: Record<string, unknown>): ScoreResult {
